...
### 3-4. 인트로 스플래시 레이아웃 표준
*   **세로 찢어짐 방지**: 기존의 `justify-between` 속성으로 인해 화면 해상도가 높을 때 헤더와 푸터가 위아래 양끝으로 벌어지는 현상을 폐기하고, 가로/세로 완전 중앙 정렬(`items-center justify-center`) 기반으로 중앙에 밀집시킵니다.
*   **이너 프레임**: 내부 콘텐츠들은 `max-w-[340px]` 크기의 이너 프레임으로 감싸서 촘촘한 `gap-y-6`로 밀집 배치합니다.
*   **디자인 톤앤매너**: Warm Cream & Gold Accents 테마에 정확히 부합하는 프리미엄 골드 나침반 일러스트(`unsu01.jpg`)를 노출하여 고급스러운 첫인상을 보장합니다.
...
### 4-3. 스크롤 락 및 바운싱 방지 패턴 (Scroll Lock & Paint Sync)
*   **스크롤 락 (Scroll Lock)**: 스플래시 화면이 노출되는 상태(`showSplash === true`)에서는 모바일 컨테이너 프레임에 동적으로 `h-[100dvh] overflow-hidden`을 부여하여 브라우저 스크롤이 작동하지 않고 화면 크기가 고정되도록 합니다.
*   **스크롤 바운싱 방지 (Paint Sync)**: 라우트 이동 및 비동기 데이터 로딩 완료 시점에 브라우저의 기본 스크롤 복원 동작 개입으로 화면이 위로 갔다가 다시 아래로 툭 떨어지는 바운싱을 예방하기 위해, 스크롤 리셋 주기를 브라우저가 화면을 그리기(Paint) 직전에 동기적으로 실행되는 `useLayoutEffect`로 전환하고 불필요한 지연 타이머(`setTimeout`)를 완전히 제거합니다.
