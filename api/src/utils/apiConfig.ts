...
export type ApiKey =
  ...
  // 교통 - 대중교통
  | 'trains'          // 코레일 KTX/일반열차
  | 'subway_seoul'    // 서울 지하철 (서울열린데이터광장)
  | 'subway_metro'    // 수도권 광역전철 (공공데이터포털 TAGO)
  // 교통 - 항공
  | 'airport'
  // 생활편의
  | 'restrooms'
  | 'opinet'
  // 문화행사 - 전국/공연예술
  | 'events_kopis'        // 공연예술통합전산망 (KOPIS) - 전국 공연
  | 'events_culture'      // 한국문화정보원 - 전국 문화행사
  // 문화행사 - 지역별
  | 'events_seoul'        // 서울시 문화행사 (서울열린데이터광장)
  | 'events_gyeonggi'     // 경기도 문화행사
  | 'events_busan'        // 부산시 관광/문화
  | 'events_incheon'      // 인천시 문화행사
  // 문화행사 - 목적별
  | 'events_sports'       // 스포츠 경기 일정 (야구/축구/농구)
  | 'events_concert'      // 대형 콘서트/페스티벌
  | 'events_convention'   // 국제 전시/컨벤션 (COEX, KINTEX 등)
  | 'events_outdoor';     // 야외 축제/행사
...
export interface ApiStatus {
  ...
  provider: string;
  group: ApiGroup;
  envKey?: string;            // .env에 넣을 변수명
  sandboxMode: boolean;
  health: 'OK' | 'ERROR' | 'UNKNOWN';
  lastChecked?: string;
  callsToday: number;
  note?: string;              // 특이사항 (무료/유료/제한 등)
}

// 초기 인메모리 설정
const configStore: Record<ApiKey, ApiStatus> = {
  // ── 인프라/지도 ────────────────────────────────────────
  address: {
...
export function getApiStatusList(): ApiStatus[] {
  return Object.values(configStore);
}

export function getApiStatusByGroup(): Record<ApiGroup, ApiStatus[]> {
  const grouped: Partial<Record<ApiGroup, ApiStatus[]>> = {};
  for (const api of Object.values(configStore)) {
    if (!grouped[api.group]) grouped[api.group] = [];
    grouped[api.group]!.push(api);
  }
  return grouped as Record<ApiGroup, ApiStatus[]>;
}

export function getApiStatus(id: ApiKey): ApiStatus {
  return configStore[id];
}

export function toggleSandboxMode(id: ApiKey, sandboxMode: boolean) {
  if (configStore[id]) {
    configStore[id].sandboxMode = sandboxMode;
  }
}

export function toggleGroupSandboxMode(group: ApiGroup, sandboxMode: boolean) {
  for (const api of Object.values(configStore)) {
    if (api.group === group) {
...
export function recordApiCall(id: ApiKey, success: boolean) {
  if (configStore[id]) {
