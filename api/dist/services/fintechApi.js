 ...
 */
export async function scrapeDailyCardRevenues(hometaxId, date) {
    try {
        if (KUCON_API_BROKER_KEY) {
            ...
            if (response.ok) {
                ...
                return rawList.map((tx) => CardRevenueTransactionSchema.parse({
 ...
 */
export async function scrapeBusinessExpenses(hometaxId, month) {
    try {
        if (KUCON_API_BROKER_KEY) {
            ...
            if (response.ok) {
                ...
                return rawList.map((ex) => BusinessExpenseTransactionSchema.parse({
                    expense_id: ex.id,
