...
위 정보를 바탕으로, 오늘의 사주 운세와 실시간 교통 상황(정체 우회 팁, 핫존 수요 등)을 반영한 기사님 맞춤형 오늘의 조언 코멘트를 3줄 내외로 작성해 주세요. 장년층 기사님이 스마트폰 거치 상태에서 흘겨봐도 즉시 읽기 편하게 반드시 친근하고 알기 쉬운 구어체 존댓말로 작성해 주셔야 합니다.`;
        ...
        if (!course) {
            ...
            if (graphState.hotzones && graphState.hotzones.length > 0) {
                ...
                if (lowerArea.includes('강남역')) {
                    lat = '37.498';
                    lon = '127.027';
                }
                else if (lowerArea.includes('김포공항')) {
                    lat = '37.558';
                    lon = '126.802';
                }
                else if (lowerArea.includes('홍대입구')) {
                    lat = '37.557';
                    lon = '126.924';
                }
                else if (lowerArea.includes('서울역')) {
                    lat = '37.555';
                    lon = '126.971';
                }
                else if (lowerArea.includes('여의도')) {
                    lat = '37.521';
                    lon = '126.924';
                }
            }
...
function getFortune(birthDate, todayStr) {
    ...
    const fortunes = [
        { grade: 'BEST', comment: '"서북쪽(마포/상암) 방향에서 장거리 손님을 만날 일진입니다. 평소보다 10분 일찍 시동을 켜보세요. 큰 재물이 따릅니다."' },
        { grade: 'GOOD', comment: '"동남쪽(강남/분당) 방면으로 주행 시 고단가 호출 성공률이 대폭 상승하는 귀인의 날입니다. 청담동 사거리 부근을 주시하세요."' },
        { grade: 'NORMAL', comment: '"대형 빌딩 밀집 지역(여의도/종로) 주변에서 퇴근길 콜 수요와 기가 막히게 매칭되는 날입니다. 마포대교 방면 우회로를 확보하세요."' },
        { grade: 'BAD', comment: '"안전한 보수 운행이 유리한 날입니다. 무리한 장거리 유혹을 삼가고 김포공항 국내선 대기열에 진입하면 안정적인 수익이 쌓입니다."' }
    ];
...
// ----------------------------------------------------
import { getApiStatusList, toggleSandboxMode, toggleGroupSandboxMode } from './utils/apiConfig.js';
...
server.post('/api/admin/external-apis/:id/toggle-sandbox', async (req, res) => {
    try {
        ...
        toggleSandboxMode(id, Boolean(sandboxMode));
        // Log this action to the admin audit logs
        const adminEmail = req.headers['x-admin-email'] || 'admin@unsu-platform.com';
        await saveAuditLog(adminEmail, 'API_SANDBOX_TOGGLE', id, `API 연동 모드를 [${sandboxMode ? 'MOCK(샌드박스)' : 'REAL(실연동)'}]로 변경했습니다.`);
...
server.post('/api/admin/external-apis/group/toggle-sandbox', async (req, res) => {
    try {
        ...
        toggleGroupSandboxMode(group, Boolean(sandboxMode));
        // Log this group action to the admin audit logs
        const adminEmail = req.headers['x-admin-email'] || 'admin@unsu-platform.com';
        await saveAuditLog(adminEmail, 'API_GROUP_SANDBOX_TOGGLE', group, `API 그룹 [${group}]의 연동 모드를 일괄 [${sandboxMode ? 'MOCK(샌드박스)' : 'REAL(실연동)'}]로 변경했습니다.`);
        res.json({ success: true, list: getApiStatusList() });
    }
    catch (err) {
        res.status(500).json({ error: err.message || err });
...
server.get('/api/external/events/aggregate', async (req, res) => {
    try {
        ...
        if (surgeOnly === 'true')
            filter.surgeOnly = true;
        if (minAttendees)
            filter.minAttendees = Number(minAttendees);
...
server.get('/api/external/opinet', async (req, res) => {
    try {
        ...
        const latitude = Number(lat) || 37.5665;
        const longitude = Number(lon) || 126.9780;
        const fuelType = (String(fuel || 'LPG'));
...
server.post('/api/admin/accounts', async (req, res) => {
    try {
        ...
        const operator = req.headers['x-admin-email'] || 'admin@unsu-platform.com';
        await saveAdmin({
            email,
            name,
            role,
        ...
        await saveAuditLog(operator, 'ADMIN_CREATE', email, `신규 관리자 계정(${name}, 권한 레벨: ${role})을 추가 등록했습니다.`);
        res.json({ success: true });
    }
    catch (err) {
        res.status(400).json({ error: err.message || err });
...
server.delete('/api/admin/accounts/:email', async (req, res) => {
    try {
        ...
        await saveAuditLog(operator, 'ADMIN_DELETE', emailToDelete, `관리자 계정(${emailToDelete})을 시스템 관리 목록에서 영구 삭제(파쇄)했습니다.`);
        res.json({ success: true });
    }
    catch (err) {
        res.status(400).json({ error: err.message || err });
...
server.post('/api/admin/accounts/:email', async (req, res) => {
    try {
        ...
        await saveAuditLog(operator, 'ADMIN_UPDATE', emailToUpdate, details);
...
server.post('/api/admin/drivers/:id', async (req, res) => {
    try {
        ...
        await saveAuditLog(adminEmail, 'DRIVER_UPDATE', id, details);
...
const handleDarksideRecommend = async (req, res) => {
    try {
