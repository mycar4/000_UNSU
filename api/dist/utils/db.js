...
// Driver Profile CRUD
// ----------------------------------------------------
export async function saveDriverProfile(driver) {
    const encryptedHometax = encrypt(driver.hometax_id);
    const naviPref = driver.navi_preference || 'TMAP';
    if (pool) {
        try {
            await pool.query(`INSERT INTO public.drivers (id, birth_date, birth_time, business_type, hometax_id, navi_preference, name, phone_number, car_model, car_number, email, address)
...
// ----------------------------------------------------
export async function getDailyLuckyCard(driverId, date) {
    if (pool) {
        try {
            const res = await pool.query('SELECT * FROM public.daily_lucky_cards WHERE driver_id = $1 AND lucky_date = $2', [driverId, date]);
            if (res.rows.length > 0) {
                return res.rows[0];
            }
            return null;
...
// ----------------------------------------------------
export async function getRecommendedCourse(driverId, date) {
    if (pool) {
        try {
            const res = await pool.query('SELECT * FROM public.recommended_courses WHERE driver_id = $1 AND target_date = $2', [driverId, date]);
            if (res.rows.length > 0) {
                return res.rows[0];
            }
            return null;
...
export async function saveRecommendedCourse(course) {
    if (pool) {
        try {
            await pool.query(`INSERT INTO public.recommended_courses (id, driver_id, target_date, destination_name, route_summary, tmap_intent_url)
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT DO NOTHING`, [course.id, course.driver_id, course.target_date, course.destination_name, course.route_summary, course.tmap_intent_url]);
            return;
        }
        catch (err) {
...
export async function updateHotZoneTime(id, waitMinutes) {
    if (pool) {
        try {
...
export async function saveLeaderboardRecord(record) {
    if (pool) {
        try {
            await pool.query(`INSERT INTO public.revenue_leaderboards (id, target_date, driver_name, route_summary, price, rank)
         VALUES ($1, $2, $3, $4, $5, $6)`, [record.id, record.target_date, record.driver_name, record.route_summary, record.price, record.rank]);
            return;
        }
        catch (err) {
    ...
    local.revenue_leaderboards = local.revenue_leaderboards.filter(l => !(l.driver_name === record.driver_name && l.target_date === record.target_date));
...
export async function updateAdmin(email, name, role) {
    if (pool) {
        try {
            await pool.query('UPDATE public.admin_accounts SET name = $2, role = $3 WHERE email = $1', [email, name, role]);
            return;
        }
        catch (err) {
...
// ----------------------------------------------------
export async function withdrawDriver(driverId) {
    ...
    if (pool) {
        try {
            ...
            await pool.query(`INSERT INTO public.withdrawn_drivers (hometax_hash, withdrawn_at)
         VALUES ($1, now())
         ON CONFLICT (hometax_hash) DO UPDATE SET withdrawn_at = now()`, [hometaxHash]);
...
export async function isRejoinRestricted(hometaxId) {
    const hometaxHash = hashHometaxId(hometaxId);
    const threeDaysAgo = new Date(Date.now() - 3 * 24 * 60 * 60 * 1000);
    if (pool) {
        try {
            const res = await pool.query('SELECT withdrawn_at FROM public.withdrawn_drivers WHERE hometax_hash = $1 AND withdrawn_at > $2', [hometaxHash, threeDaysAgo]);
            if (res.rows.length > 0) {
...
// ----------------------------------------------------
export async function saveAuditLog(adminEmail, actionType, targetIdentifier, description) {
...
`;
    ...
    if (pool) {
        try {
            await pool.query(`INSERT INTO public.admin_audit_logs (admin_email, action_type, target_identifier, description)
         VALUES ($1, $2, $3, $4)`, [adminEmail, actionType, targetIdentifier, description]);
            return;
        }
        catch (err) {
...
export async function getFinancialRecord(driverId, month) {
    if (pool) {
        try {
            const res = await pool.query('SELECT * FROM public.financial_records WHERE driver_id = $1 AND record_month = $2', [driverId, month]);
            if (res.rows.length > 0) {
                return {
                    id: res.rows[0].id,
...
export async function saveFinancialRecord(record) {
    if (pool) {
        try {
            await pool.query(`INSERT INTO public.financial_records (id, driver_id, record_month, total_revenue, fixed_expense)
         VALUES ($1, $2, $3, $4, $5)
         ON CONFLICT (driver_id, record_month) 
         DO UPDATE SET total_revenue = $4, fixed_expense = $5`, [record.id, record.driver_id, record.record_month, record.total_revenue, record.fixed_expense]);
            return;
        }
        catch (err) {
...
export async function saveTaxRefund(refund) {
    if (pool) {
        try {
            await pool.query(`INSERT INTO public.tax_refunds (id, driver_id, request_date, status, estimated_refund_amount, processed_at)
         VALUES ($1, $2, $3, $4, $5, $6)`, [refund.id, refund.driver_id, refund.request_date, refund.status, refund.estimated_refund_amount, refund.processed_at || null]);
            return;
        }
        catch (err) {
...
export async function getTaxRefunds(driverId) {
    if (pool) {
        try {
            const res = await pool.query('SELECT * FROM public.tax_refunds WHERE driver_id = $1 ORDER BY request_date DESC', [driverId]);
            return res.rows.map(r => ({
                id: r.id,
